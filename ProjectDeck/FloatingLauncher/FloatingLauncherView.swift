    ...
    private func handleKeyEvent(_ event: NSEvent) -> Bool {
        ...
        default:
            ...
            if let num = numberKeyToSlotIndex(event.keyCode) {
                let isNewTab = (flags.contains(.control) && flags.contains(.command)) || flags.contains(.command)
                launchFolder(at: num, inNewTab: isNewTab)
                return true
            }
        }
        return false
...
private struct RadialHitTestOverlay: NSViewRepresentable {
    ...
    class RadialTrackerView: NSView {
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            let isNewTab = isControlCommand || isCommand
            if let index = calculateSegmentIndex(at: loc) {
                onClick?(index, isNewTab)
            } else {
        ...
        override func rightMouseDown(with event: NSEvent) {
            let flags = event.modifierFlags.union(NSEvent.modifierFlags)
            let isControlCommand = flags.contains(.control) && flags.contains(.command)
            // If Control+Command is held, macOS might convert click to rightMouseDown.
            // In that case, handle as new tab launch instead of opening context menu.
            if isControlCommand {
                let loc = convert(event.locationInWindow, from: nil)
                if let index = calculateSegmentIndex(at: loc) {
                    onClick?(index, true)
